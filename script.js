(() => {
  ...
  const firstMusicGesture = () => {
    if(!state.musicStarted) startMusic();
  };
  window.addEventListener("pointerdown", firstMusicGesture, {once:true});
  window.addEventListener("keydown", firstMusicGesture, {once:true});
  ...
  // Pointerdown fires for mouse, touch and pen BEFORE click.
  // Moving here means every physical attempt to press No relocates it.
  ...
  $("hugVisual").addEventListener("click",openHug);
  $("hugClose").addEventListener("click",closeHug);
  $("hugNext").addEventListener("click",()=>{closeHug();showScene("candle")});
