(() => {
  ...
  const finalLetter = finalSection?.querySelector(".letter");
  const finalProgressFill = $("finalProgressFill");
  const finalWhisper = $("finalWhisper");
  ...
  function pulseSceneShift(){
    if(matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    body.classList.remove("scene-shift");
    void body.offsetWidth;
    body.classList.add("scene-shift");
    setTimeout(() => body.classList.remove("scene-shift"), 560);
  }
  ...
  function showScene(id){
    ...
    pulseSceneShift();
    window.scrollTo(0,0);
    if(id === "final") burstParticles(36);
    if(id === "final") initFinalExperience();
    if(id === "candle") resetCandle();
  }

  function updateFinalProgress(){
    if(!finalSection || !finalLetter || !finalSection.classList.contains("active")) return;

    const rect = finalLetter.getBoundingClientRect();
    const start = innerHeight * 0.78;
    const span = rect.height + innerHeight * 0.5;
    const progress = Math.max(0, Math.min(1, (start - rect.top) / Math.max(1, span)));

    if(finalProgressFill){
      finalProgressFill.style.transform = `scaleX(${progress})`;
    }

    if(finalWhisper){
      if(progress < 0.3) finalWhisper.textContent = "take your time with this";
      else if(progress < 0.72) finalWhisper.textContent = "every line means it";
      else finalWhisper.textContent = "keep this close, always";
    }
  }

  function initFinalExperience(){
    if(!finalSection) return;

    finalSection.classList.add("final-enhanced");

    const revealNodes = [
      ...finalSection.querySelectorAll(".letter-body p"),
      ...finalSection.querySelectorAll(".letter-body .hand-list"),
      ...finalSection.querySelectorAll(".letter-body .wish-list"),
      ...finalSection.querySelectorAll(".letter-body .final-punch")
    ];
    ...
    if(matchMedia("(prefers-reduced-motion: reduce)").matches || !("IntersectionObserver" in window)){
      ...
      finalObserver = new IntersectionObserver((entries) => {
      ...
      revealNodes.forEach(node => finalObserver.observe(node));
    }

    updateFinalProgress();
  }

  if(finalLetter && !matchMedia("(prefers-reduced-motion: reduce)").matches && matchMedia("(hover: hover) and (pointer: fine)").matches){
    finalLetter.addEventListener("pointermove", (event) => {
      if(!finalSection.classList.contains("active")) return;
      const rect = finalLetter.getBoundingClientRect();
      const x = (event.clientX - rect.left) / rect.width;
      const y = (event.clientY - rect.top) / rect.height;
      finalLetter.style.setProperty("--tilt-x", `${(x - 0.5) * 1.8}deg`);
      finalLetter.style.setProperty("--tilt-y", `${(0.5 - y) * 1.4}deg`);
    });

    finalLetter.addEventListener("pointerleave", () => {
      finalLetter.style.setProperty("--tilt-x", "0deg");
      finalLetter.style.setProperty("--tilt-y", "0deg");
    });
  }

  addEventListener("scroll", updateFinalProgress, {passive:true});
  addEventListener("resize", updateFinalProgress);
  ...
  function moveNo(event){
    ...
    const minX = pad;
    const minY = pad;
    const maxX = Math.max(minX, viewportW - btnW - pad);
    const maxY = Math.max(minY, viewportH - btnH - pad);
    ...
    const fromX = Number.isFinite(lastX) ? lastX : currentRect.left;
    const fromY = Number.isFinite(lastY) ? lastY : currentRect.top;

    const minMove = Math.max(120, Math.min(viewportW, viewportH) * 0.35);
    const minMoveSq = minMove * minMove;

    const farCandidates = candidates.filter(candidate => {
      const dx = candidate.x - fromX;
      const dy = candidate.y - fromY;
      return (dx*dx + dy*dy) >= minMoveSq;
    });

    let best = candidates[0];
    if(farCandidates.length){
      best = farCandidates[Math.floor(Math.random() * farCandidates.length)];
    }else{
      // Fallback for very small screens: pick the farthest candidate available.
      let bestDistance = -1;
    ...
    const x = Math.min(maxX,Math.max(minX,best.x));
    const y = Math.min(maxY,Math.max(minY,best.y));

    const localX = x - zoneRect.left;
  ...
  // Desktop hover-capable pointers should dodge on hover.
  ...
  // Universal activation path: desktop click and mobile tap.
  ...
  $("celebrate").addEventListener("click",()=>{
    burstParticles(42);
    const frame = finalSection?.querySelector(".final-frame");
    if(!frame) return;
    frame.classList.remove("celebrate-pop");
    void frame.offsetWidth;
    frame.classList.add("celebrate-pop");
  });
